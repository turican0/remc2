...
void GameRenderNG::DrawSky_40950_old(int16_t roll/*, uint8_t startLine, uint8_t drawEveryNthLine*/)
{
	int v1; // ebx
	int v2; // edx
	int v3; // esi
...
void GameRenderNG::DrawSky_40950_new(int16_t roll)
{
...
void GameRenderNG::DrawTerrainAndParticles_3C080(__int16 posX, __int16 posY, __int16 yaw, signed int posZ, int pitch, int16_t roll, int fov)
{
	...
	*/
	...
	v248x[32] = (v9 >> 9) & 3;
	v248x[30] = Maths::x_DWORD_DB750[512 + v12];
...
int32_t* GameRenderNG::x_DWORD_DB350_ret(uint32_t address) {
	if (address >= 256) return &Maths::x_DWORD_DB750[address - 256];
	allert_error();
}
