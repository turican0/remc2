...
static int udf_extract_files(udf_t *p_udf, udf_dirent_t *p_udf_dirent, const char *psz_path)
{
	...
	while (udf_readdir(p_udf_dirent)) {
		...
		/*i_length = */sprintf_s(psz_fullpath, i_length, "%s%s/%s", (char*)psz_extract_dir, (char*)psz_path, (char*)psz_basename);
		i_length = strlen(psz_fullpath);
		if (i_length < 0) {
			goto out;
		}
		printf("-- Extracting: %s\n", psz_fullpath);
...
void cd_iso_extract(char* exfile, char* exdir)
//int main(int argc, char** argv)
{
	...
	//const char* exdir = "c:\\prenos\\ex";
	//const char* exfile = "c:\\prenos\\gparted-live-0.27.0-1-i686.iso";
...
try_iso:
	p_iso = iso9660_open_ext(exfile, ISO_EXTENSION_ALL);
	if (p_iso == NULL) {
