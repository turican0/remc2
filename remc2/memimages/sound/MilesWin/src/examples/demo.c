...
HSAMPLE     DigitalStreamSampleHandle = 0;   // Underlying HSAMPLE for stream
...
F32         DigitalVolume = 1.0f;             // Current volume
...
static void Open_my_MSS_resources()
{
   ...
   DigitalStreamSampleHandle = AIL_stream_sample_handle(DigitalStream);
   ...
   if (DigitalStream)
   {
     AIL_set_stream_user_data( DigitalStream,
                               0,
                               AIL_sample_playback_rate( DigitalStreamSampleHandle ) );
   }
...
static void Start_digital_sound( U32 which )
{
  ...
  for (i = 0; i < DigitalCount; i++)
  {
    if (AIL_sample_status( DigitalSamples[i] ) == SMP_DONE)
    {
      ...
      //
      // First, point the sample handle at the loaded file image
      //
      ...
      AIL_set_sample_volume_pan( DigitalSamples[i], DigitalVolume, 0.5f );
      ...
      AIL_set_sample_playback_rate( DigitalSamples[i], (S32)
                                    ((DigitalPitch + 1L)
...
static void Play_CD_track( S32 offset )
{
  U32 start, end;
...
void Show_about( HWND wnd )
{
   ...
   lstrcat(text," " MSS_COPYRIGHT "\n\n"
               ...
               "Audio samples courtesy of Rapid Eye Entertainment" );
...
static S32 Update_pitch( S32 newpos )
{
  ...
  for (i = 0;i < DigitalCount;i++)
  {
    ...
    if (AIL_sample_status( DigitalSamples[i] ) == SMP_PLAYING)
    {
      ...
      AIL_set_sample_playback_rate( DigitalSamples[i],
                              (((U32)DigitalPitch + 1L)
                              * (U32) AIL_sample_user_data( DigitalSamples[i], 0 ))
                              / 64L);
    }
  }
  ...
  if (DigitalStream)
    AIL_set_sample_playback_rate( DigitalStreamSampleHandle,
                                  (((U32)DigitalPitch + 1L)
                                  * (U32) AIL_stream_user_data( DigitalStream, 0 ))
                                  / 64L);
  return( DigitalPitch );
}
...
static S32 Update_volume( S32 newpos )
{
  ...
  DigitalVolume = ( (F32) newpos ) / 256.0f;
  ...
  if (DigitalStream)
   AIL_set_sample_volume_pan( DigitalStreamSampleHandle, DigitalVolume, 0.5f );
                         ...
                         HREDBOOK       red )
{
  ...
  SetScrollRange( ctrl, SB_CTL, 0, 256, FALSE );
  SetScrollPos( ctrl, SB_CTL, 256, TRUE );
