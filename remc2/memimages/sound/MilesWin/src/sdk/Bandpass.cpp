...
#define FILTER_NAME "BandPass Filter"

// Center Frequency / Width Default
#define  _FX_CENTER_DEFAULT      2000.0F
#define  _FX_WIDTH_DEFAULT       400.0F
...
static void  FXCalcParams( SAMPLESTATE FAR * SS, S32 playback_rate )
{
   ...
   fC =  1.0F / (F32) AIL_tan( F_PI * SS->fWidth / fRate );
   fD =  2.0F * (F32) AIL_cos( 2.0F * F_PI * SS->fCenterFreq / fRate );
...
static S32 AILCALL FLTSMP_sample_property(HSAMPLESTATE state, HPROPERTY property, void FAR * before_value, void const FAR * new_value, void FAR * after_value )
{
   ...
   switch ( property )
   {
     ...
     case _FX_BANDPASS_WIDTH:
       if ( before_value ) *(F32 FAR*)before_value = SS->fWidth;
       if ( new_value ) 
       {
         SS->fWidth = *(F32 const FAR*)new_value;
         // clip to valid range
         FX_CLIPRANGE( SS->fWidth, 20.0F, ((F32) SS->driver->dig->DMA_rate) / 2.0F - 1.0F );
         FXCalcParams( SS, SS->calculated_rate);
       }         
       if ( after_value ) *(F32 FAR*)after_value = SS->fWidth;
       return 1;
