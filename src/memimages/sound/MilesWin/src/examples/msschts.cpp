...
static void show_string(char* buf)
{
  U32 i;
  if (scrolllen>4096)
  {
...
DWORD WINAPI CLIENT_transmit_thread(LPVOID pParam)
{
...
DWORD WINAPI CLIENT_receive_thread(LPVOID pParam)
{
   char buf[128];
      ...
      switch (C->state)
         ...
         case WAITING_FOR_DATA_ID:
            ...
            RIB_INTERFACE_ENTRY ASICODEC[] =
               ...
               PR("Maximum frame size", FSIZE),
               };

            if (RIB_request(C->HPRECV,"ASI codec",ASICODEC) != RIB_NOERR)
               {
               show_string("Frame size property not available, upgrade your ASI codec!");
            ...
            C->RECV_frame_size = 0;
            PROVIDER_property(FSIZE, &C->RECV_frame_size, 0, 0 );
            ...
            C->XMIT_frame_size = 0;
            PROVIDER_property(FSIZE, &C->XMIT_frame_size, 0, 0 );
            ...
            RIB_INTERFACE_ENTRY XMITSTR[] = 
               {
               { RIB_FUNCTION,   "ASI_stream_property",       (UINTa) &XMIT->ASI_stream_property,       RIB_NONE },
               { RIB_FUNCTION,   "ASI_stream_open",           (UINTa) &XMIT->ASI_stream_open,           RIB_NONE },
               { RIB_FUNCTION,   "ASI_stream_seek",           (UINTa) &XMIT->ASI_stream_seek,           RIB_NONE },
               { RIB_FUNCTION,   "ASI_stream_close",          (UINTa) &XMIT->ASI_stream_close,          RIB_NONE },
               { RIB_FUNCTION,   "ASI_stream_process",        (UINTa) &XMIT->ASI_stream_process,        RIB_NONE },
               { RIB_PROPERTY,   "Output sample rate",        (UINTa) &XMIT->OUTPUT_SAMPLE_RATE,        RIB_NONE },
               { RIB_PROPERTY,   "Output sample width",       (UINTa) &XMIT->OUTPUT_BITS,               RIB_NONE },
               { RIB_PROPERTY,   "Output channels",           (UINTa) &XMIT->OUTPUT_CHANNELS,           RIB_NONE },
               { RIB_PROPERTY,   "Requested sample rate",     (UINTa) &XMIT->REQUESTED_RATE,            RIB_NONE },
               };
            ...
            RECV->stream = RECV->ASI_stream_open((UINTa) C,
                                                         RECV_stream_CB,
                                                         0);
            ...
            //
            // Stream is now open -- get its property and set output
            ...
            XMIT->stream = XMIT->ASI_stream_open((UINTa) C,
                                                         XMIT_stream_CB,
                                                         0);
            ...
            XMIT->ASI_stream_property( XMIT->stream,
                                       XMIT->REQUESTED_RATE,
                                       0, &req_rate, 0 );
...
S32 CLIENT_connect(sockaddr_in *who, SOCKET requestor)
{
   ...
   C->receive_thread = CreateThread(NULL,
                                    0,
                                    CLIENT_receive_thread,
                                    (PVOID) C,
                                    0,
   ...
   C->transmit_thread = CreateThread(NULL,
                                     0,
                                     CLIENT_transmit_thread,
                                     (PVOID) C,
                                     0,
...
DWORD WINAPI MONITOR_thread_procedure(LPVOID pParam)
{
   char buf[128];
                   ...
                   LPSTR lpszCmdLine, int nCmdShow)
{
   ...
   HANDLE monitor_thread = CreateThread(NULL,
                                        0,
                                        MONITOR_thread_procedure,
                                        NULL,
                                        0,
