...
MSS_STRUCT SYNTHDEVICE FAR *DLS;
...
static U32 in_chunk(CHUNK FAR *current, CHUNK FAR *outer)
{
   ...
   CHUNK FAR *end = (CHUNK FAR *) AIL_ptr_add(outer, LEU_SWAP32( &outer->ckSize ) + 8 + ( LEU_SWAP32( &outer->ckSize ) & 1));
...
static void ART1_parse(INSTRUMENT *INS, U32 index, CHUNK FAR *c1)
{
   ...
   CONNECTIONLIST FAR *cl = (CONNECTIONLIST FAR *)(void FAR*) (((SUBCHUNK FAR *) c1)->data);

   CONNECTION FAR *c = (CONNECTION FAR *) (((U8 FAR *) cl) + LEU_SWAP32( &cl->cbSize ) );
...
//            INS->region_list[index].header = *(RGNHEADER FAR *) (((SUBCHUNK FAR *) c2)->data);
         ...
         case FOURCC_WSMP:
            {
            WSMPL FAR *ptr = (WSMPL FAR *) (void FAR*)(((SUBCHUNK FAR *) c2)->data);
...
//            INS->region_list[index].sample = *ptr;
            ...
            if ( INS->region_list[index].sample.cSampleLoops )
               ...
               INS->region_list[index].loop =
                  *(WLOOP FAR *) (((U8 FAR *) ptr) + LEU_SWAP32( &ptr->cbSize ) );
...
static void INS_parse(DLS_FILE FAR*file, CHUNK FAR *c1)
{
   ...
   INSTRUMENT FAR*INS = (INSTRUMENT FAR*) ILM_alloc(instrument_list);
...
static void WAVE_parse(DLS_FILE FAR*file, CHUNK FAR *c1, UINTa offset, U32 flags)
{
   flags = flags; // for watcom's benefit

#if defined( IS_WIN32API ) || defined( IS_MAC )
   if (flags & AILDLSLIST_DUMP_WAVS)
      ...
      CHUNK FAR *c1_end = (CHUNK FAR *) AIL_ptr_add(c1, LEU_SWAP32( &c1->ckSize ) + 8 + (LEU_SWAP32( &c1->ckSize ) & 1));
      ...
      while (c2 < c1_end)
         ...
         if ((c2->ckID == FOURCC_LIST) && (c2->subID == FOURCC_INFO))
            {
            SUBCHUNK FAR *c2_end = (SUBCHUNK FAR *) AIL_ptr_add(c2, LEU_SWAP32( &c2->ckSize ) + 8 + (LEU_SWAP32( &c2->ckSize ) & 1));
            ...
            while (c3 < c2_end)
               ...
               c3 = (SUBCHUNK FAR *) AIL_ptr_add(c3, LEU_SWAP32( &c3->ckSize ) + 8 + (LEU_SWAP32( &c3->ckSize ) & 1));
               }
            }

         c2 = (CHUNK FAR *) AIL_ptr_add(c2, LEU_SWAP32( &c2->ckSize ) + 8 + (LEU_SWAP32( &c2->ckSize ) & 1));
         }
...
S32 DLSFILE_parse(void const FAR *data, DLS_FILE FAR * FAR *file_var, C8 const FAR *lpFileName, U32 flags)
{
   ...
   DLS_FILE FAR*file = (DLS_FILE FAR*) FLM_alloc(file_list, lpFileName);
   ...
   CHUNK FAR *c2 = (CHUNK FAR *)(void FAR*) outer->data;
   ...
   while (in_chunk(c2,outer))
      ...
      switch (c2->ckID)
         ...
         case FOURCC_PTBL:
            ...
            file->cues = (POOLCUE   FAR *) AIL_ptr_alloc_clone(&((U8 FAR *) (file->ptbl))[LEU_SWAP32( &file->ptbl->cbSize )]);
            break;
         }
   ...
   c2 = (CHUNK FAR *)(void FAR*) outer->data;
   ...
   while (in_chunk(c2,outer))
      ...
      switch (c2->ckID)
         ...
         case FOURCC_COLH:
            ...
            file->cInstruments = (S32) LEU_SWAP32( (((SUBCHUNK FAR *) c2)->data) );
            break;
            ...
            switch (c2->subID)
               ...
               case FOURCC_WVPL:
                  ...
                  while (in_chunk(c3,c2))
                     ...
                     #else
                        WAVE_parse(file, c3, (U32) AIL_ptr_dif(c3, (U8 FAR *) c2->data), flags);
                     #endif
                        }
