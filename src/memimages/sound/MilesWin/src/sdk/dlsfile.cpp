...
static S32 get_expanded_DLS_size(void FAR *MLS, S32 MLS_size)
{
   ...
   CHUNK FAR *n1 = (CHUNK FAR *) AIL_ptr_add(MLS, LEU_SWAP32( &c1->ckSize ) + 8 + (LEU_SWAP32( &c1->ckSize) & 1));
...
static int expand(void FAR *out, S32 FAR *out_size,AILLENGTHYCB callback)
{
   ...
   CHUNK FAR *n1 = (CHUNK FAR *) AIL_ptr_add(out, LEU_SWAP32( &c1->ckSize ) + 8 + (LEU_SWAP32( &c1->ckSize ) & 1));
   CHUNK FAR *c2 = (CHUNK FAR *) AIL_ptr_alloc_clone(c1->data);
   ...
   while (AIL_ptr_lt(c2,n1))
      ...
      if (c2->ckID == FOURCC_PTBL)
         ...
         cues = (POOLCUE   FAR *) AIL_ptr_alloc_clone(&((U8 FAR *) (ptbl))[LEU_SWAP32( &ptbl->cbSize ) ]);
         break;
         }
...
//      debug_printf("No PTBL chunk found\n");
   ...
   n1 = (CHUNK FAR *) AIL_ptr_add(out, LEU_SWAP32( &c1->ckSize ) + 8 + ( LEU_SWAP32( &c1->ckSize ) & 1));
            ...
            if ((c3->ckID == FOURCC_LIST) && (c3->subID == FOURCC_wave))
               ...
               while (AIL_ptr_lt(c4,n3))
                  ...
                  if (c4->ckID == FOURCC_PCM)
                     ...
                     total_samples = LEU_SWAP32( &((IMA_FACT FAR *) c4)->samples );
                     }
...
//                        debug_printf("Insufficient free memory to expand file\n");
                     ...
                     if (need_ASI == 1)
                        ...
                        if (!decode_ASI(ASI_chunk.original_file_suffix,
                                        s,
                                        LEU_SWAP32( &((IMA_DATA FAR*) c4)->chunk_size ),
                                        data->data,
                        ...
                        RIB_request(HP,"MSS mixer services",MIXER);
                         ...
                         S16 FAR *d = (S16 FAR*)(void FAR*)data->data;
                        ...
                        MIXER_adpcm_decode(&d,&s, size, LEU_SWAP32( &((IMA_DATA FAR*)c4)->chunk_size ),
                     ...
                     void FAR *end = AIL_ptr_add(out, LEU_SWAP32( &c1->ckSize ) + 8 + (LEU_SWAP32( &c1->ckSize ) & 1));

                     CHUNK FAR *nxt = (CHUNK FAR *) AIL_ptr_alloc_clone(c4);
                     AIL_ptr_inc_clone(nxt, old_size);

                     CHUNK FAR *dest = (CHUNK FAR *) AIL_ptr_alloc_clone(c4);
                                  ...
                                  AILLENGTHYCB      callback)
{
   ...
   if (is_IFF)
      ...
      if (XDLS_start != NULL)
         ...
         XLS_size  = BEU_SWAP32( ((U8 FAR *) (XDLS_start+4)));
         }
      }
   else
      {
...
extern "C" S32 AILCALL AIL_API_file_type_named(void const FAR* data, char const FAR* filename, U32 size)
{
  if ( filename )
  {
                                       ...
                                       void FAR* FAR*dls, U32 FAR* dlssize)
{
  DOLOCK();
   ...
   if (((CHUNK FAR*)data)->ckID == FOURCC_RIFF)
      ...
      if ((((CHUNK FAR*)data)->subID == FOURCC_DLS) ||
         ...
         if (dlssize) *dlssize = LEU_SWAP32( &((CHUNK FAR *) data)->ckSize) + 8;
         return 1;
         }
      }
  ...
  if ((!AIL_strnicmp((char FAR*) (data)  , "FORM", 4)) &&
    ...
    if (XDLS_start != NULL)
       ...
       if (dlssize)
         *dlssize=LEU_SWAP32( &(((CHUNK FAR*)XDLS_start)->ckSize ) )+8;
