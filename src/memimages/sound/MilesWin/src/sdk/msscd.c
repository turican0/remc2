...
static SINTa voldeviceid=-1;  // holds an aux device or an HMIXER
...
static HREDBOOK rbcompleteopen(U32 which, U32 flags, MCI_OPEN_PARMS *pmciopen)
{
  ...
  AIL_serve();
  if (mciSendCommand((WORD)which,MCI_OPEN,flags,(DWORD_PTR)(LPVOID)pmciopen)) {
    AIL_serve();
    if (mciSendCommand((WORD)which,MCI_OPEN,flags|MCI_OPEN_SHAREABLE,(DWORD_PTR)(LPVOID)pmciopen))
      return(0);
  }

  AIL_serve();
  mciset.dwTimeFormat=MCI_FORMAT_MILLISECONDS;
  mciSendCommand(pmciopen->wDeviceID,MCI_SET,MCI_SET_TIME_FORMAT,(DWORD_PTR)(LPVOID)&mciset);
  AIL_serve();
...
U32 AILCALL AIL_API_redbook_status(HREDBOOK hand)
{
  ...
  if (hand) {
    ...
    AIL_serve();
    mciSendCommand((UINT)hand->DeviceID,MCI_STATUS,MCI_STATUS_ITEM|MCI_WAIT,(DWORD_PTR)(LPVOID)&mcistatus);
    AIL_serve();
...
void AILCALL AIL_API_redbook_track_info(HREDBOOK hand,U32 tracknum,U32 FAR* startmsec,U32 FAR* endmsec)
{
  ...
  mciSendCommand((UINT)hand->DeviceID,MCI_STATUS,MCI_STATUS_ITEM|MCI_TRACK|MCI_WAIT,(DWORD_PTR)(LPVOID)&mcistatus);
  AIL_serve();
  start=(U32)mcistatus.dwReturn;
  if (startmsec)
    *startmsec=start;
  ...
  if (endmsec) {
    ...
    *endmsec=start+(U32)mcistatus.dwReturn;
  }
}
...
U32 AILCALL AIL_API_redbook_play(HREDBOOK hand,U32 startmsec, U32 endmsec)
{
  ...
  if (AIL_redbook_status(hand)!=REDBOOK_ERROR) {
    ...
    mciSendCommand((UINT)hand->DeviceID,MCI_PLAY,MCI_FROM|MCI_TO,(DWORD_PTR)(LPVOID)&mciplay);
  } else {
    return(REDBOOK_ERROR);
  }
  ...
  if (hand)
    if (voldeviceid!=-1) {
      ...
      if (mixerapi) {
        ...
        return((volumevalue[0].dwValue&0xffff)/65535.0f);
...

      {
        ...
        if (auxGetVolume((UINT)voldeviceid,(DWORD*)&volume)==0)
          return((volume&0xffff)/65535.0f);
      }
  ...
  if ((hand) && (volume>=0) && (volume<=1.0f))
    if (voldeviceid!=-1)  {
      ...
      if (mixerapi) {
        ...
        volumevalue[0].dwValue=(S32)(volume*65535.0f);
        volumevalue[1].dwValue=(S32)(volume*65535.0f);
