...
void FAR *AIL_ptr_alloc_clone(void const FAR *src)
{
   ...
   ptr = (void FAR *) (((UINTa)(AllocSelector(HIWORD(src))) << 16) + LOWORD(src));
...
S32 AIL_ptr_dif(void const FAR *p1, void const FAR *p2)
{
  if (HIWORD(p1)==HIWORD(p2))
    return((SINTa)((UINTa)LOWORD(p1))-(SINTa)((UINTa)LOWORD(p2)));

   return (SINTa)(((SINTa)(GetSelectorBase(HIWORD(p1)) + LOWORD(p1))) -
                  ((SINTa)(GetSelectorBase(HIWORD(p2)) + LOWORD(p2))));
}
...
void FAR * AILCALLBACK w32_alloc(UINTa size)
{
   void FAR* p;
   ...
   up = (void FAR*) GlobalAlloc( GMEM_FIXED | GMEM_ZEROINIT, size + 32 );
   if ( up == 0 )
   {
...
void AILCALLBACK w32_free( void FAR * ptr )
{
  void FAR * p;

  p = ( ( void FAR * FAR * ) ptr )[ -1 ];
  ( ( void FAR * FAR * ) ptr )[ -1 ] = 0;
  ...
  GlobalFree( (HANDLE) p );
      ...
      if (p==0)
      {
        ...
        OutputDebugString("Miles detected memory error - Double freed memory...\n");
#else
        MessageBox(0,"Double freed memory.","Miles detected memory error",MB_OK|MB_ICONSTOP);
#endif
      }
      else
      {
...
void FAR * AILCALL AIL_API_mem_alloc_lock(UINTa size)
{
#ifdef IS_WIN32API
