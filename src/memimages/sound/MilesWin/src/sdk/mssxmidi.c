                                     ...
                                     S32        size)
{
   ...
   size = (size - ((S32) AIL_ptr_dif(ptr, message))) + 1;
   ...
   *(U8 FAR *) mdi->mhdr->lpData = *message;
   ...
   size = min(size, (S32) AIL_preference[MDI_SYSEX_BUFFER_SIZE]);
...
void AILLIBCALLBACK XMI_serve(HMDIDRIVER mdi) // WINAPI because same DS as caller
{
   //
            ...
            while ((!sequence_done) && ((status = *S->EVNT_ptr) >= 0x80))
               ...
               switch (status)
                  ...
                  case EV_META:
                     ...
                     switch (type)
                        ...
                        case META_TIME_SIG:
                           ...
                           q = 16000000L / (S32) AIL_preference[MDI_SERVICE_RATE];
                  ...
                  case EV_ESC:
                     ...
                     len += (S32) AIL_ptr_dif(ptr, S->EVNT_ptr);
...
void init_mdi_defaults(HMDIDRIVER mdi)
{
   ...
   for (i=0; i < NUM_CHANS; i++)
      ...
      XMI_MIDI_message(mdi,EV_CONTROL | i,
                                        PART_VOLUME,
                                        (S32) AIL_preference[MDI_DEFAULT_VOLUME]);
      ...
      XMI_MIDI_message(mdi,EV_CONTROL | i,
                                        DATA_MSB,
                                        (S32) AIL_preference[MDI_DEFAULT_BEND_RANGE]);
...
#endif
{
   ...
   mdi->n_sequences = (S32) AIL_preference[MDI_SEQUENCES];
   ...
   mdi->interval_time = 1000000L / (S32) AIL_preference[MDI_SERVICE_RATE];
   ...
   mdi->sysdata=AIL_mem_alloc_lock((S32) AIL_preference[MDI_SYSEX_BUFFER_SIZE]);
   ...
   AIL_memset(mdi->sysdata,0, (S32) AIL_preference[MDI_SYSEX_BUFFER_SIZE]);

   //
   // Prepare sysex header and buffer for use
   //

   mdi->mhdr->lpData         = (LPSTR) mdi->sysdata;
   mdi->mhdr->dwBufferLength = (S32) AIL_preference[MDI_SYSEX_BUFFER_SIZE];
   ...
   AIL_set_timer_frequency(mdi->timer,(S32) AIL_preference[MDI_SERVICE_RATE]);
...
S32 AILCALL AIL_API_init_sequence(HSEQUENCE S, void const FAR *start, S32 sequence_num)
{
   ...
   while (image < end)
      {
      if (!AIL_strnicmp((char FAR*) image,"TIMB",4))
         {
         S->TIMB = image;
         }

      if (!AIL_strnicmp((char FAR*) image,"RBRN",4))
         {
         S->RBRN = image;
         }

      if (!AIL_strnicmp((char FAR*) image,"EVNT",4))
         {
         S->EVNT = image;
         }
   ...
   S->volume         =  (S32) AIL_preference[MDI_DEFAULT_VOLUME];
   S->volume_target  =  (S32) AIL_preference[MDI_DEFAULT_VOLUME];
   S->volume_period  =  0;
   S->volume_accum   =  0;
   ...
   AIL_memcpy(TIMB,
        S->TIMB,
           min(sizeof(TIMB),
               8 + BE_SWAP32( ((U8 FAR *) ((char FAR *) S->TIMB+4)))));
...
void AILCALL AIL_API_sequence_position(HSEQUENCE S, S32 FAR *beat, S32 FAR *measure)
{
   ...
   for (i=0; i < (S32) AIL_preference[MDI_QUANT_ADVANCE]; i++)
      {
      f += S->time_fraction;
...
void AILCALL AIL_API_branch_index(HSEQUENCE S, U32 marker)
{
   ...
   if ((S32) AIL_preference[MDI_ALLOW_LOOP_BRANCHING] == NO)
      {
...
void AILCALL AIL_API_send_sysex_message(HMDIDRIVER mdi, void const FAR *buffer)
{
   ...
   len += (S32) AIL_ptr_dif(ptr, buffer);
                                  ...
                                  S32 FAR   *current_milliseconds)
{
   ...
   if (total_milliseconds != NULL)
      {
      *total_milliseconds = (1000*int_cnt)/(S32)AIL_get_preference(MDI_SERVICE_RATE);
      }

   if (current_milliseconds != NULL)
      {
      *current_milliseconds = (1000*current)/(S32)AIL_get_preference(MDI_SERVICE_RATE);
      }
}
                                            ...
                                            S32       milliseconds)
{
   ...
   S32 target_interval=(milliseconds*(S32)AIL_get_preference(MDI_SERVICE_RATE))/1000;
   S32 done = 0;
