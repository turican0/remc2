...
void OPL::change_attackrate(Bitu regbase, op_type* op_pt) {
...
void OPL::change_decayrate(Bitu regbase, op_type* op_pt) {
...
void OPL::change_releaserate(Bitu regbase, op_type* op_pt) {
...
void OPL::change_sustainlevel(Bitu regbase, op_type* op_pt) {
...
void OPL::change_waveform(Bitu regbase, op_type* op_pt) {
...
void OPL::change_keepsustain(Bitu regbase, op_type* op_pt) {
...
void OPL::change_vibrato(Bitu regbase, op_type* op_pt) {
...
void OPL::change_feedback(Bitu chanbase, op_type* op_pt) {
	Bits feedback = adlibreg[ARC_FEEDBACK+chanbase]&14;
	if (feedback) op_pt->mfbi = (Bit32s)(pow(FL2,(fltype)((feedback>>1)+8)));
	else op_pt->mfbi = 0;
}

void OPL::change_frequency(Bitu chanbase, Bitu regbase, op_type* op_pt) {
	// frequency
...
void OPL::enable_operator(Bitu regbase, op_type* op_pt, Bit32u act_type) {
...
void OPL::disable_operator(op_type* op_pt, Bit32u act_type) {
...
void OPL::adlib_init(int samplerate) {
	Bits i, j, oct;
...
void OPL::adlib_write(Bitu idx, Bit8u val) {
...
void OPL::adlib_getsample(Bit16s* sndptr, Bits numsamples) {
	Bits i, endsamples;
	op_type* cptr;
