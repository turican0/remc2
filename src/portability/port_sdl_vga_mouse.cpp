...
bool VGA_LoadFont()
{
	...
	{
		Logger->error(fmt::runtime("Unable to load image {}! SDL Error: {}"), "16x16-font.bmp", SDL_GetError());
		success = false;
	}
	...
	if (m_surfaceFont == nullptr)
	{
		Logger->error(fmt::runtime("Unable to load image {}! SDL Error: {]"), "8x8-font.bmp", SDL_GetError());
		success = false;
	}
