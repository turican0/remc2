...
ssize_t DataFileIO::ReadFileAndDecompress(const std::filesystem::path& path, uint8_t** data)
{
	ssize_t result = 0; // eax // length of file or 0 on error
	FILE* file; // ebx
	uint32_t length; // esi
	file = CreateOrOpenFile(path, 0x200);
...
ssize_t DataFileIO::Decompress(uint8_t* src, uint8_t* a2) {
	vars_t* v = init_vars();
	if (v->method == 1)
	{
...
{
	off_t start_pos = v->input_offset;
...
ssize_t DataFileIO::Unpack(vars_t* v)
{
	...
	ssize_t result = UnpackData(v); // data
	if (result == 0)return v->input_size;
	return -result;
}

FILE* DataFileIO::CreateOrOpenFile(const std::filesystem::path& pathname, int __pmode)
{
	FILE* file; // ST10_4
...
FILE* DataFileIO::CreateFile(const std::filesystem::path& path, uint32_t flags)
{
	return mycreate(path, flags);
}

FILE* DataFileIO::Open(const std::filesystem::path& path, int pmode, uint32_t flags) {
...
ssize_t DataFileIO::FileLengthBytes(FILE* file) 
{
	ssize_t size;
