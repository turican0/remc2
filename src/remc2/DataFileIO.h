...
public:
	static ssize_t ReadFileAndDecompress(const std::filesystem::path& path, uint8_t** data); //Reads the file path passed in into "data" array of 1 byte elements
	static ssize_t Decompress(uint8_t* a1, uint8_t* a2);
	static vars_t* init_vars();
	static int UnpackData(vars_t* v);
	static ssize_t Unpack(vars_t* v);
	static FILE* CreateOrOpenFile(const std::filesystem::path& pathname, int __pmode);
	static FILE* CreateFile(const std::filesystem::path& path, uint32_t flags);
	static FILE* Open(const std::filesystem::path& path, int pmode, uint32_t flags);
	...
	static ssize_t FileLengthBytes(FILE* file);
