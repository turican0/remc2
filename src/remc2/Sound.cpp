...
void sub_8D290_init_sound(/*char* a1*//*, int a2, int a3*/)//26e290
{
	...
	std::filesystem::path soundGamePath = settings.gameDataPath / "SOUND";
	//fix it
	//v7 = 0;
	//v5 = 0;
	//fix it
	...
	if (v7)
	{
		sscanf((char* const)v7, "%s %x %d %d %d", (uintptr_t)(&v6));
	...
	else
	{
		...
		return; // sound fixed*/
		std::filesystem::path digPath = soundGamePath / "DIG.INI";
...
void /*__fastcall*/ sub_8D970_init_music(/*char* a1*//*int a1, int a2, char* a3*/)//26e970
{
	...
	std::filesystem::path soundCdPath;
	std::filesystem::path soundGamePath;

	soundCdPath   = settings.cdDataPath / "SOUND";
	soundGamePath = settings.gameDataPath / "SOUND";
	...
	{
		std::filesystem::path musicMdiPath = soundGamePath / "MDI.INI";
...
//----- (00093330) --------------------------------------------------------
HDIGDRIVER sub_93330_AIL_install_DIG_driver_file(/*int a1, */const char* filename, IO_PARMS* IO)//AIL_install_DIG_driver_file // 274330
{
	//AIL_fix();
...
//----- (00095710) --------------------------------------------------------
int32_t sub_95710_AIL_install_MDI_INI(HMDIDRIVER* mdi, const std::filesystem::path& fileName)//HMDIDRIVER *mdi AIL_install_MDI_INI //276710
{
	//AIL_fix();
	int i; // [esp+0h] [ebp-10h]
	int j; // [esp+0h] [ebp-10h]
...
//----- (00095850) --------------------------------------------------------
HMDIDRIVER sub_95850_AIL_install_MDI_driver_file(const char* filename, IO_PARMS* IO)//HMDIDRIVER *mdi AIL_install_MDI_driver_file //276850
{
	//AIL_fix();
	int i; // [esp+0h] [ebp-10h]
	int j; // [esp+0h] [ebp-10h]
...
int32_t sub_84300_load_sound(uint8_t a1)//265300
{
	...
	if (x_BYTE_E3798_sound_active2 && x_BYTE_E37AD_actual_sound != a1)
	{
		...
		std::string soundPath = GetSubDirectoryFile(settings.cdFolder, "SOUND", "SOUND.DAT");
...
void LoadSoundDataFromBuffer_844A0(uint16_t count)//2654a0
{
	...
	if (soundIndex_E37A0 && x_DWORD_E37A8_sound_buffer1)
	{
		...
		while (index < count)
		{
			...
			//v1 += 32;
			soundIndex_E37A0->str_8.wavs_10[index].wavData_0 = (uintptr_t)(soundIndex_E37A0->str_8.wavs_10[index].wavData_0) + x_DWORD_E37A8_sound_buffer1;
...
//----- (0009E3A0) --------------------------------------------------------
int sub_9E3A0_AIL_API_read_INI(AIL_INI* INI, const std::filesystem::path& filename/*,char* a8*/)//27f3a0
{
	//unsigned int v2; // eax
	//unsigned int v3; // eax
	//unsigned int v4; // eax
	...
	*/
	//fix
	if (!filename.filename().compare("DIG.INI"))
	{
		strcpy(INI->device_name, "Creative Labs Sound Blaster 16 or AWE32");
		strcpy(INI->driver_name, "SB16.DIG");
		std::filesystem::path workingDir = GetDirectory(filename);
		sprintf(INI->driver_path, "%s/%s", workingDir.c_str(), "SB16.DIG");
	...
	if (!filename.filename().compare("MDI.INI"))
	{
		strcpy(INI->device_name, "Creative Labs Sound Blaster(TM) 16");
		strcpy(INI->driver_name, "SBPRO2.MDI");
		std::filesystem::path workingDir = GetDirectory(filename);
		sprintf(INI->driver_path, "%s/%s", workingDir.c_str(), "SBPRO2.MDI");
		INI->IO.IO = sub_9E2B0((char*)"220h", 16);
		INI->IO.IRQ = sub_9E2B0((char*)"-1", 10);
		INI->IO.DMA_8_bit = sub_9E2B0((char*)"-1", 10);
		INI->IO.DMA_16_bit = sub_9E2B0((char*)"-1", 10);
...
//----- (000A3600) --------------------------------------------------------
HDIGDRIVER sub_A3600_AIL_API_install_DIG_driver_file(/*int a1,*/ const char* filename, IO_PARMS* IO)//284600
{
	...
	if (v6)
	{
		v3 = sub_9DE20_get_file_length(filename);
...
//----- (000A3720) --------------------------------------------------------
int sub_A3720_AIL_API_install_DIG_INI(/*int a1, */HDIGDRIVER* dig, const std::filesystem::path& digPath)//284720
{
...
HSAMPLE sub_A4970(int a1, uint8_t* a2, int a3)//285970
{
	...
	{
		v6->sam_var[542] = *(unsigned __int16*)(a2 + 20) + (uintptr_t)(a2);
		v6->sam_var[545] = a3;
		v6->sam_var[546] = a3 == -1;
		v6->sam_var[547] = 1;
...
//----- (000A77D0) --------------------------------------------------------
HMDIDRIVER sub_A77D0_AIL_API_install_MDI_INI(const char* filename, IO_PARMS* IO)//2887d0
{
	int v3; // eax
...
//----- (000A78F0) --------------------------------------------------------
int32_t sub_A78F0_AIL_API_install_MDI_INI(HMDIDRIVER* mdi, const std::filesystem::path& fileName) //2888f0
{
...
//----- (000931F0) --------------------------------------------------------
int sub_931F0_AIL_install_DIG_INI(/*int a1,*/ HDIGDRIVER* dig, const std::filesystem::path& digPath)//AIL_install_DIG_INI //2741f0
{
	//AIL_fix();
	...
	result = sub_A3720_AIL_API_install_DIG_INI(/*a1,*/ dig, digPath.c_str());
	if (x_DWORD_181BF4 && (x_DWORD_181C04 == 1 || x_DWORD_181BF8) && !sub_A16A2())
	{
		for (i = 0; i < 0xE; i++)
			dbgfprintf(x_DWORD_181BF0_AIL_debugfile, " \0");
...
signed int LoadMusic(int channel)//26fad0
{
	...
	std::filesystem::path musicPath = settings.cdDataPath / "SOUND/MUSIC.DAT";
	if (!x_BYTE_E37FC_music)
		return 0;
...
//----- (00092190) --------------------------------------------------------
int LoadAILFromFile(AIL_INI* INI, const std::filesystem::path& filename/*, char* a8, IO_PARMS IO*/)//AIL_read_INI // 273190
{
