...
void InitTmaps(unsigned __int16 a1)//251f50
{
	...
	std::filesystem::path tmapsdirpost;
	...
	if (settings.bigSprites)
	{
		...
		if (big_sprites_inited)
		{
			spritePath = settings.bigGraphicsPath;
		}
		else
		{
			spritePath = settings.gameDataPath;
		}

		switch (D41A0_0.terrain_2FECE.MapType) {
		case MapType_t::Day: {
			tmapsdirpost = spritePath / "TMAPS/TMAPS2-0-";
			break;
		}
		case MapType_t::Night: {
			tmapsdirpost = spritePath / "TMAPS/TMAPS2-1-";
			break;
		}
		case MapType_t::Cave: {
			tmapsdirpost = spritePath / "TMAPS/TMAPS2-2-";
			break;
		}
		}
	}
					...
					if (settings.bigSprites)
					{
						...
						if ((oldwidth > 0) && (oldwidth < 512) && (oldheight > 0) && (oldheight < 512))
						{
							...
							for (mm = 0; mm < max_sprites_frames; mm++)
							{
								char filebuffer[512];
								//FILE* fptr_outdata;
								sprintf(filebuffer, "%s%03d-%02d.data", tmapsdirpost.c_str(), i, mm);
