...
DataFileRNC::DataFileRNC() {};

int DataFileRNC::Decompress(uint8_t* src, uint8_t* a2) {
	vars_t* v = init_vars();
	if (v->method == 1)
	{
...
DataFileRNC::vars_t* DataFileRNC::init_vars()
{
...
int DataFileRNC::UnpackData(vars_t* v)
{
...
int DataFileRNC::Unpack(vars_t* v)
{
...
uint8_t DataFileRNC::read_byte(uint8_t* buf, size_t* offset)
{
	return buf[(*offset)++];
}

uint16_t DataFileRNC::read_word_be(uint8_t* buf, size_t* offset)
{
...
uint32_t DataFileRNC::read_dword_be(uint8_t* buf, size_t* offset)
{
...
int DataFileRNC::unpack_data_m1(vars_t* v)
{
...
int DataFileRNC::unpack_data_m2(vars_t* v)
{
	while (v->processed_size < v->input_size)
	{
		while (1)
...
void DataFileRNC::decode_match_count(vars_t* v)
{
...
void DataFileRNC::decode_match_offset(vars_t* v)
{
	v->match_offset = 0;
	if (input_bits_m2(v, 1))
	{
...
uint16_t DataFileRNC::crc_block(uint8_t* buf, size_t offset, int size)
{
	uint16_t crc = 0;
...
uint32_t DataFileRNC::input_bits_m1(vars_t* v, short count)
{
	uint32_t bits = 0;
...
uint32_t DataFileRNC::input_bits_m2(vars_t* v, short count)
{
	uint32_t bits = 0;
...
int DataFileRNC::input_bits(vars_t* v, short count)
{
	if (v->method != 2)
...
void DataFileRNC::write_decoded_byte(vars_t* v, uint8_t b)
{
...
void DataFileRNC::write_buf(uint8_t* dest, size_t* offset, uint8_t* source, int size)
{
	memmove(&dest[*offset], source, size);
	*offset += size;
}

uint8_t DataFileRNC::read_source_byte(vars_t* v)
{
...
void DataFileRNC::read_buf(uint8_t* dest, uint8_t* source, size_t* offset, int size)
{
	memmove(dest, &source[*offset], size);
	*offset += size;
}

void DataFileRNC::ror_w(uint16_t* x)
{
	if (*x & 1)
		*x = 0x8000 | (*x >> 1);
	else
...
uint32_t DataFileRNC::decode_table_data(vars_t* v, huftable_t* data)
{
	uint32_t i = 0;
...
void DataFileRNC::clear_table(huftable_t* data, int count)
{
